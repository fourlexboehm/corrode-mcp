...
use crate::vendor::probe::models::{LimitedSearchResults, SearchLimits, SearchResult};
...
fn process_matches(
    ...
    for (file_path, term_map) in file_matches {
        ...
        for (start_line, end_line) in blocks {
            ...
            let code_lines = &file_lines
                [(start_line - 1).min(file_lines.len() - 1)..(end_line).min(file_lines.len())];
    ...
    // Sort results
    results.sort_by(|a, b| a.file.cmp(&b.file).then_with(|| a.lines.0.cmp(&b.lines.0)));
...
pub fn apply_limits(
    ...
    let total_tokens: usize = results
        .iter()
        .map(|r| r.code.split_whitespace().count())
        .sum();
